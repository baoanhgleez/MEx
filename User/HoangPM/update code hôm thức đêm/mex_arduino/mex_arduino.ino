...
const int RELEASE_PEDAL_SPEED = 1023;
const int MAXPUSH_PEDAL_BRAKE = 520;
const int RELEASE_PEDAL_BRAKE = 1023;
...
class Indicator
{
  ...
  Buzzer *buzzer;
	...
	Indicator(byte btnLeft, byte btnRigh, byte ledLeft, byte ledRigh, Buzzer *buzzer)
	{
...
class Controller
{
   ...
   this->indicator = (new Indicator(btnLeft, btnRigh, ledLeft, ledRigh,buzzer));
	...
	int getSpeed()
	{
		if ((mode == 0) || (analogRead(handBrake) <500)) 
		{
        
//      if(analogRead(handBrake) < 3)
//        buzzer->buzz(100);
//      delay(10);
...
//    delay(10);
		if (pedalInput < abs(RELEASE_PEDAL_SPEED)) // -5 la sai so trong qua trinh lay du lieu
		{
			// co nhan ga
			speed = map(pedalInput, RELEASE_PEDAL_SPEED, MAXPUSH_PEDAL_SPEED, 20, 100);
		}
		pedalInput = analogRead(pedalBrake);
//    delay(10);
		if (pedalInput < abs(RELEASE_PEDAL_BRAKE))
		{
			// co nhan phanh
