...
class MExManager():
    ...
    def __init__(self, sock_info, devices):
        ...
        ##
        self._token = {'ANDROID':0, 'ARDUINO':0}
        self._require = False
    ...
    def disconnect(self, client, address, devName, reason):
        ...
        logf('Close connection to '+str(address)) 
        client.close()
    ...
    def listenToArduino(self, client, address):
        while True:
            ...
            try:
                ...
                self.disconnect( client, address, 'ARDUINO', 'Time out!')
                break
            ...
            try:
                ...
                if (self.__ARDUINO_SET == set(order.keys()) ):
                    ...
                    if led == 1:
                        logf('Indicator LEFT','ARDUINO')
                    elif led == 2:
                        logf('Indicator RIGHT','ARDUINO')
                    if buzz>0:
                        logf('BUZZ BUZZ','ARDUINO')
            except ValueError:
                logf('Invalid JSON string!','ERROR')
